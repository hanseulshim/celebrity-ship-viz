...
const getStatus = (point, colorCount, peerGroupFilters) => {
	...
	if (!peerGroupFilters) {
		...
		statusObj.selectedPercent = point.selectedPercent
			? point.selectedPercent
			: 'N/A'
		statusObj.peerGroupPercent = point.peerGroupPercent
			? point.peerGroupPercent
			: 'N/A'
		...
		const difference =
			parseInt(point.difference) >= 20
				? 20
				: parseInt(point.difference) <= -20
				? -20
				: parseInt(point.difference)
		const offset = 20
...
export const createDeck = (data, deck, selectedDeck, peerGroupFilters) => {
	...
	const opacity = deck === selectedDeck ? 1 : 0.1
	return {
		x,
		y,
		z,
